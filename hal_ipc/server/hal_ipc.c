    ...
    while ((cli_sock = accept(p_ipc_node->srv_sock, (struct sockaddr *)&cli_sockaddr, &len)) != -1) {
        ...
        if (sizeof(hal_ipc_processor_desc_t) > max_size) {
            while (target_bytes < sizeof(hal_ipc_processor_desc_t)) {
                if ((nbytes = recv(cli_sock, ptr, (sizeof(hal_ipc_processor_desc_t) - target_bytes > max_size) ? max_size : (sizeof(hal_ipc_processor_desc_t) - target_bytes), 0)) == -1) {
                    wifi_hal_error_print("%s:%d:receiving command response failed err: %d\n", __func__, __LINE__, errno);
                    close(cli_sock);
                    break_cycle = 1;
                    break;
                }
            ...
            if ((nbytes = recv(cli_sock, (unsigned char *) &desc, sizeof(hal_ipc_processor_desc_t), 0)) == -1) {
                wifi_hal_error_print("%s:%d:receiving command response failed err: %d\n", __func__, __LINE__, errno);
                close(cli_sock);
                continue;
            }
        ...
        if (desc.len > sizeof(hal_ipc_processor_desc_t)) {
            ...
            if (desc.scratch_buf_size > max_size) {
                while (target_bytes < desc.scratch_buf_size) {
                    if ((nbytes = recv(cli_sock, (unsigned char *)tmp, (desc.scratch_buf_size - target_bytes) > max_size ? max_size : (desc.scratch_buf_size - target_bytes), 0)) == -1) { 
                        wifi_hal_error_print("%s:%d:receiving desc scratch buf in chunks failed err: %d\n", __func__, __LINE__, errno);
                        free(desc.scratch_buf);
                        close(cli_sock);
                        break_cycle = 1;
                        break;
                ...
                if ((nbytes = recv(cli_sock, (unsigned char *)tmp, desc.scratch_buf_size, 0)) == -1) {
                    wifi_hal_error_print("%s:%d:receiving command response failed err: %d\n", __func__, __LINE__, errno);
                    free(desc.scratch_buf);
                    close(cli_sock);
                    continue;
