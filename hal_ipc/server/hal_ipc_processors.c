...
unsigned char *client_scratch_buf = NULL;

//--------------------------------------------------------------------------------------------------
static void cleanup_client_data(void)
{
                      ...
                      void *arg5)
{
    ...
    switch (desc->type) {
        ...
        case hal_ipc_desc_type_get_radio_chan_stats:
            ...
            input_output_channelStats_array = (wifi_channelStats_t *) malloc(HAL_IPC_RADIO_CHANNELS_MAX*sizeof(wifi_channelStats_t));
            ...
            // call OneWifi monitor functions to collect wifi_neighbor_ap2_t
            app_get_radio_channel_stats_t callback_fn_chan_stats = hal_ipc_server_get_radio_channel_stats_callback();
            if (!callback_fn_chan_stats) {
                wifi_hal_error_print("%s:%d FAIL %s callback is NULL.\n", __func__, __LINE__, desc->name);
                free(input_output_channelStats_array);
                goto error_happened;
            }

            desc->ret = callback_fn_chan_stats(index, input_output_channelStats_array, &array_size);
        ...
        case hal_ipc_desc_type_get_neighbor_wifi_status:
            ...
            wifi_neighbor_ap2_t *neighbor_ap_array = (wifi_neighbor_ap2_t*) malloc(HAL_IPC_MAX_NEIGHBOR_AP_COUNT*sizeof(wifi_neighbor_ap2_t));
            if (!neighbor_ap_array) {
                wifi_hal_error_print("%s:%d FAIL to allocate memory for %d wifi_neighbor_ap2_t array.\n", __func__, __LINE__, HAL_IPC_MAX_NEIGHBOR_AP_COUNT);
                goto error_happened;
            }

            // call OneWifi monitor functions to collect wifi_neighbor_ap2_t
            app_get_neighbor_ap2_t callback_fn_neighbor_ap = hal_ipc_server_get_neighbor_ap2_callback();
            if (!callback_fn_neighbor_ap) {
                wifi_hal_error_print("%s:%d FAIL %s callback is NULL.\n", __func__, __LINE__, desc->name);
                cleanup_desc_scratch_buf(desc);
                goto error_happened;
            }

            desc->ret = callback_fn_neighbor_ap(index, neighbor_ap_array, &output_array_size);
                                  ...
                                  void *arg5)
{
    ...
    switch (desc->type) {
        ...
        case hal_ipc_btm_response_event:
            ...
            desc->scratch_buf = (unsigned char*) malloc(sizeof(wifi_BTMResponse_t));
            if (!desc->scratch_buf) {
                wifi_hal_dbg_print("%s:%d FAIL desc->scratch_buf malloc %s for wifi_BTMResponse_t\n", __func__, __LINE__, desc->name);
                return -1;
            }
            ...
            desc->scratch_buf_size = sizeof(wifi_BTMResponse_t);
            desc->len = sizeof(hal_ipc_processor_desc_t) + desc->scratch_buf_size;
            break;
